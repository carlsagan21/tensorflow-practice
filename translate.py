...
# tf.app.flags.DEFINE_boolean("ckpt_clear", False, "Ignore existing checkpoints.")
...
def read_data(source_path, target_path, max_size=None):
    ...
    print("  reading data line %d" % (counter if counter else "undef"))
    return data_set
...
def train():
    ...
    with tf.Session() as sess:
        ...
        while is_continue:
            ...
            # health check
            check_term = 10
            if current_step % check_term == 0:
                perplexity = math.exp(float(step_loss)) if loss < 300 else float("inf")
                print("global step %d learning rate %.4f step-time %.2f loss %.2f perplexity "
                      "%.2f" % (model.global_step.eval(), model.learning_rate.eval(),
                                time.time() - start_time, step_loss, perplexity))
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                checkpoint_path = os.path.join(FLAGS.train_dir,
                                               "translate.ckpt" + ".size." + str(FLAGS.size) + ".nl."
...
def decode_tester(sess, model):
    ...
    print("\toutput: " + " ".join([tf.compat.as_str(rev_fr_vocab[output]) for output in outputs]))
    sys.stdout.flush()
